...
// Type: Nop.Plugin.InstantSearch.Dotless.Parser.Functions.ComplementFunction
...
namespace Nop.Plugin.InstantSearch.Dotless.Parser.Functions
{
  public class ComplementFunction : HslColorFunctionBase
  {
    ...
    {
      this.WarnNotSupportedByLessJS("complement(color)");
      color.Hue += 0.5;
      return (Node) color.ToRgbColor();
    }

    protected override Node EditHsl(HslColor color, Number number) => (Node) null;
  }
}
