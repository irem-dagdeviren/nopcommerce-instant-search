...
// Type: Nop.Plugin.InstantSearch.Dotless.Parser.Functions.RedFunction
...
namespace Nop.Plugin.InstantSearch.Dotless.Parser.Functions
{
  ...
  {
    protected override Node Eval(Color color) => (Node) new Number(color.RGB[0]);

    protected override Node EditColor(Color color, Number number)
    {
      this.WarnNotSupportedByLessJS("red(color, number)");
      double num = number.Value;
      if (number.Unit == "%")
        num = num * (double) byte.MaxValue / 100.0;
      return (Node) new Color(color.R + num, color.G, color.B);
    }
  }
}
