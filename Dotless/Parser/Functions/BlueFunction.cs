...
// Type: Nop.Plugin.InstantSearch.Dotless.Parser.Functions.BlueFunction
...
namespace Nop.Plugin.InstantSearch.Dotless.Parser.Functions
{
  ...
  {
    protected override Node Eval(Color color) => (Node) new Number(color.B);

    protected override Node EditColor(Color color, Number number)
    {
      this.WarnNotSupportedByLessJS("blue(color, number)");
      double num = number.Value;
      if (number.Unit == "%")
        num = num * (double) byte.MaxValue / 100.0;
      return (Node) new Color(color.R, color.G, color.B + num);
    }
  }
}
